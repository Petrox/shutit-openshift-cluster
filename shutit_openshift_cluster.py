...
class shutit_openshift_cluster(ShutItModule):
  ...
  "dns": ["8.8.8.8"]
		...
		while not shutit_session.send_until('oc get pods | grep ^router- | grep -v deploy | grep Running','.*Running.*',cadence=30,retries=10):
			shutit_session.send('oc deploy router --retry')
		while not shutit_session.send_until('oc get pods | grep ^docker-registry- | grep -v deploy | grep Running','.*Running.*',cadence=30,retries=10):
